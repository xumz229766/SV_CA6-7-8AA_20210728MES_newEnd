...
namespace Motion.AdlinkDash
{
    ...
    ///     凌华科技 7230 IO 卡控制器。
    /// </summary>
    public class DaskController_7230 : Automatic, ISwitchController, INeedInitialization, IDisposable
    {
        private readonly byte[] _cardNos;
        private readonly Dictionary<int, short> _devices;
        private bool _disposed;

        public DaskController_7230(params byte[] cardNos)
        {
            _cardNos = cardNos;
        ...
        public void Initialize()
        {
            ...
            {
                var device = DASK.Register_Card(DASK.PCI_7230, cardNo);
                if (device < 0)
                {
                    switch (device)
                    {
