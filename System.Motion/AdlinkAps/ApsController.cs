...
namespace Motion.AdlinkAps
{
    ...
    public class ApsController : Automatic, ISwitchController, IMotionController, INeedInitialization, IDisposable
    {
        ...
        public bool IsLoadXmlFile2 { get; private set; }
        ...
        public bool LoadParamFromFile1(string xmlfilename)
        {
            if (!_isInitialized) return false;
            IsLoadXmlFile2 = (APS168.APS_load_param_from_file(xmlfilename) == 0);
            if (IsLoadXmlFile2)
            {
                //AutoClosingMessageBox.Show("Load XML File OK !", "204C208C", 2000);
            }
        ...
        {
            var code = 0;
            if (!_isInitialized) return (StopReasons)code;
        ...
        {
            var status = 0;
            if (!_isInitialized) return (GearModes)status;
            ...
            if (ret == 0)
            {
                foreach (var cardNo in _cardNos)
                {
                        ...
                        (cardName == (int) APS_Define.DEVICE_NAME_AMP_20408C))
                    {
                        ...
                        _isInitialized = true;//xmz
                    }
                    else
                    {
                        _isInitialized = false;//xmz
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            //_isInitialized = false;//xmz20200914
            _disposed = true;
        }
