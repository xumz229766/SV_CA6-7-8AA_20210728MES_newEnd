    ...
    public partial class frmWhiteBorad : Form
    {
        ...
        public void StartOneTest()
        {
            ...
            finally
            {
                ...
                log.Debug($"白板测试结果{Marking.WhiteBoardResult} Image{lblCapImage.Text} Bad{ lblBad.Text} Stain{lblStain.Text}");
        ...
        public void ImageThread()
        {
            ...
            IntPtr GrabBuffer = Marshal.AllocHGlobal((int)Size);
            IntPtr pBmp = Marshal.AllocHGlobal(width * height * 4);
                ...
                else
                {
                    if (DTCCM2_API.WriteSensorReg((byte)_slave, _sReg, _sValue, (byte)_sIIMode, this._deviceID) != WhiteBoardDetection.DT_ERROR_OK)
                    {
                        if (DTCCM2_API.WriteSensorReg((byte)_slave, _sReg, _sValue, (byte)_sIIMode, this._deviceID) != WhiteBoardDetection.DT_ERROR_OK)
                        {
                            ...
                            log.Debug("WriteSensorReg：" + _slave.ToString()+","+_sReg.ToString()+","+_sValue.ToString()+","+ _sIIMode.ToString());
                            WriteSensorRegflag = true;
                        }
                    }
                }
            }
            if (WriteSensorRegflag) { return; }
        ...
        public int OneTest()
        {
            ...
            if (resDefect == 1) { lblBad.Text = "OK"; Marking.WhiteBoardResult_Detect = true; }
            else { lblBad.Text = "NG"; Marking.WhiteBoardResult_Detect = false; }

            if (resStain == 1) { lblStain.Text = "OK"; Marking.WhiteBoardResult_Stain = true; }
            else { lblStain.Text = "NG"; Marking.WhiteBoardResult_Stain = true; }
