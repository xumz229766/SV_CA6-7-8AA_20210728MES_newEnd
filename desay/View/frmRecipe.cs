    ...
    public partial class frmRecipe : Form
    {
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show(string.Format("是否删除型号：{0}", deleteType), "提示", MessageBoxButtons.YesNo) == DialogResult.Yes)
            {
                ...
                RefreshFileList();
                //添加删除AA和白板文件夹2021.4.7
                try
                {
                    if (Directory.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"Ref\\{deleteType}")))
                        Directory.Delete(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"Ref\\{deleteType}"), true);                  
                }
                catch { }
                try
                {
                    if (Directory.Exists(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"WhiteRef\\{deleteType}")))
                        Directory.Delete(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, $"WhiteRef\\{deleteType}"), true);
                }
                catch { }
            }
        }
        ...
        private void btnSwitch_Click(object sender, EventArgs e)
        {
            ...
            m_Load?.Invoke(); ;
            RefreshInfo();
            MessageBox.Show($"机种已更新为{CurrentProductType}");
        }
