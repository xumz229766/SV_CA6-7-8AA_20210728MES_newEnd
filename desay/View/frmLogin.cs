...
namespace desay
{
    public partial class frmLogin : Form
    {
        ...
        private void btnLogin_Click(object sender, EventArgs e)
        {
            ...
            if ( cbxUers.Text == UserLevel.管理员.ToString())
            {
                if (SecurityHelper.TextToMd5(txtPassword.Text) == Config.Instance.SuperUserPassword)
                {
                    Config.Instance.userLevel = UserLevel.管理员;
                    Config.Instance.userName = cbxUers.Text;
                    log.Info(string.Format("用户 {0} 已登录", Config.Instance.userName));
                    this.DialogResult = DialogResult.OK;
                    this.Close();
